...
let unitary_gates : [`unitary_gates] structure typ = structure "unitary_gates"
let first_gate = field unitary_gates "first_gate" (final_App1)
let second_gate= field unitary_gates "second_gate" (final_App2)
let third_gate= field unitary_gates "third_gate" (final_App3)
let () = seal unitary_gates

let get_gate_apps d = 
let v =getf d first_gate in
let b =getf d second_gate in
let n =getf d third_gate in
(v, b, n)
let set_gate_apps (v, b, n) =
let d = make unitary_gates in (setf d first_gate v;setf d second_gate b;setf d third_gate n;d)
let final_gates_act = view ~read:get_gate_apps~write:set_gate_apps unitary_gates
